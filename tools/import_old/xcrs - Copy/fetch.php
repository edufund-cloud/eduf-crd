    ...
    if(!empty($_FILES['csv_file']['name'])){
        ...
        while($row = fgetcsv($file_data)){
            $row_data[] = array(
                ...
                'nama_penyelenggara'  => $row[52],
                'nama_program'  => $row[53],
                'alamat_penyelenggara'  => $row[54],
                'telepon_penyelenggara'  => $row[55],
                'tanggal_pembayaran'  => $row[56],
                'total_pembiayaan'  => $row[57],
                'uang_muka'  => $row[58],
                'tenor'  => $row[59],
                'cicilan_per_bulan'  => $row[60],
                'bank_tujuan'  => $row[61],
                'nama_rekening'  => $row[62],
                'nomor_rekening'  => $row[63]
            );
        }
        $output = array(
            'column'  => $column,
