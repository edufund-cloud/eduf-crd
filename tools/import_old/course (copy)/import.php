    ...
    if(isset($_POST["id_debitur"])){
        ...
        for($count = 0; $count < count($id_debitur); $count++){
            ...
            $sql = "INSERT INTO apply(ID, full_name, apply_date, 
                ...
                '$product[$count]', '$pic_name', 'Borrower',
            ...
            if(mysqli_query($koneksi, $sql)){
                ...
                if ($product[$count] == "Uang Kursus"){
                    $sql_crd = "INSERT INTO apply_course(data_id, nama_penyelenggara, nama_program, 
                        alamat, telepon, 
                        ...
                        '$tanggal_pembayaran[$count]', '$total_pembiayaan[$count]', '$uang_muka[$count]', 
                        '$tenor[$count]', '$cicilan_per_bulan[$count]', 
                    ...
                    $sql_crd = "INSERT INTO apply_kta(data_id, total_pembiayaan, tenor, biaya_admin,
                        ...
                        ('$new_data_id', '$total_pembiayaan[$count]', '$tenor[$count]', '$uang_muka[$count]',
                        '$total_pencairan[$count]', '$cicilan_per_bulan[$count]', '$bank_tujuan[$count]',
                        '$nama_rekening[$count]', '$nomor_rekening[$count]')";
                    $xsql_crd = mysqli_query($koneksi, $sql_crd);
                }

                //Data Lampiran                
                if (!empty($lampiran_ktp[$count])){
                    $sql_atc = "INSERT INTO apply_attach(data_id,description,attach_url,data_update,user_update) VALUES 
                        ('$new_data_id','KTP','$lampiran_ktp[$count]','$now','$user_id')";
                    $xsql_atc = mysqli_query($koneksi, $sql_atc);
                }                
                if (!empty($lampiran_npwp[$count])){
                    $sql_atc = "INSERT INTO apply_attach(data_id,description,attach_url,data_update,user_update) VALUES 
                        ('$new_data_id','NPWP','$lampiran_npwp[$count]','$now','$user_id')";
                    $xsql_atc = mysqli_query($koneksi, $sql_atc);
                }                
                if (!empty($lampiran_gaji[$count])){
                    $sql_atc = "INSERT INTO apply_attach(data_id,description,attach_url,data_update,user_update) VALUES 
                        ('$new_data_id','Slip Gaji','$lampiran_gaji[$count]','$now','$user_id')";
                    $xsql_atc = mysqli_query($koneksi, $sql_atc);
                }
                if (!empty($lampiran_kk[$count])){
                    $sql_atc = "INSERT INTO apply_attach(data_id,description,attach_url,data_update,user_update) VALUES 
                        ('$new_data_id','KK','$lampiran_kk[$count]','$now','$user_id')";
                    $xsql_atc = mysqli_query($koneksi, $sql_atc);
                }
                if (!empty($lampiran_ktp_pasangan[$count])){
                    $sql_atc = "INSERT INTO apply_attach(data_id,description,attach_url,data_update,user_update) VALUES 
