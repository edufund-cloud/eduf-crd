    ...
    <div class="row mb-2">
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-left">
          ...
          <li class="breadcrumb-item active">Data Credit - PV</li>
        </ol>
      </div>    
    </div>
  </div><!-- /.container-fluid -->
  ...
  <div class="container-fluid">
    <div class="card card-lightblue card-outline" style="border-radius:12px;">
      <div class="card-header">
        <h3 class="card-title text-navy font-weight-bold"><i class="fa fa-list mr-2"></i>Data Credit - PV</h3>
      </div>
      <!-- /.card-header -->
      <div class="card-body">
          ...
          <tbody>
            <?PHP
              $sql = "SELECT a.data_id,a.ID,a.full_name, 
                ...
                ORDER BY a.data_status, a.apply_date, a.data_update DESC";
              //echo $sql;
              $xsql = mysqli_query($koneksi,$sql);
              $no =1;
              while($arsql = mysqli_fetch_array($xsql)){
                if ($arsql['data_status'] == "Pefindo Done"){
                  $Class_Status = "badge badge-info" ;
                  $Label_Status = "New";
                }
                else if ($arsql['data_status'] == "Pending PV"){
                  $Class_Status = "badge bg-warning" ;
                  $Label_Status = "Pending";
                }
                else if ($arsql['data_status'] == "Send Back PV"){
                  $Class_Status = "badge badge-danger" ;
                  $Label_Status = "Recheck";
                } 
